...
export default async function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen">
      <header className="sticky top-0 z-20 border-b border-orange-200 bg-merca-creamSoft/95 backdrop-blur">
          ...
          {links.map((link) => {
            ...
            return (
              <Link
                key={link.href}
                href={link.href}
                className="inline-flex min-h-10 shrink-0 items-center gap-2 rounded-lg bg-white px-3 text-sm font-black text-merca-ink shadow-sm"
              >
